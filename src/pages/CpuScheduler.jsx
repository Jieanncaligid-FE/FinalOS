...
const CpuScheduler = () => {
  ...
  const roundRobin = (list, quantum) => {
    ...
    const avgWT = (waiting.reduce((acc, value) => acc + value, 0) / n).toFixed(2);
    const avgTAT = (turnaround.reduce((acc, value) => acc + value, 0) / n).toFixed(2);
    ...
    <section className="cpu-page">
      <div className="container">
        ...
        <div className="input-group">
          ...
          <input name="arrival" type="number" placeholder="Arrival Time" value={form.arrival} onChange={handleChange} min="0" />
          <input name="burst" type="number" placeholder="Burst Time" value={form.burst} onChange={handleChange} min="1" />
          <input type="number" placeholder="Time Quantum" value={timeQuantum} onChange={(event) => setTimeQuantum(event.target.value)} min="1" />
        ...
        {error && <p className="metrics" style={{ color: '#d92c21' }}>{error}</p>}
        ...
        <div className="gantt-chart">
          ...
          {!gantt.length && <span className="no-processes">Run a calculation to view the chart.</span>}
        </div>
      </div>
    </section>
  );
